...
from index.yaml', self.name)
			...
			# TODO - add more repos. this is why hub obj 
			...
			# the url we will query for all the dockerhub info
...
size=100')
			...
			while (image_url is not None):
				...
				if (image_obj.exist_in_repo):
					...
					# If dealing w/ container and 
					#haven't gotten archs
					if (image_obj.container \
					in image_obj.tags \
					and len(image_obj.archs) == 0):
						image_obj.is_container = True
						# Now the container is part 
						#of image_obj
						image_obj.get_archs()
...
organization', self.name, self.org)
 ...
 %s', self.name, self.tags[-1])
	...
	def get_archs(self):
		...
		wanted_tag = next(tag for tag in results if tag["name"] \
== self.container)
