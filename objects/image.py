...
class App:
	...
	def get_product_name(self, dest_dir):
		...
		with open(readme_loc, 'r') as readme:
			readme_lines = readme.readlines()	#get list of lines in file
			regex = re.compile("^# ")		#make regex string looking for "# " at the beginning of a line
			for line in readme_lines:		#search all of the lines
				if (regex.search(line) != None):
					#If the regex got a hit make the product name a sanitize version of the line
					self.product_name = line.replace('#','').strip()
					return

		
		self.product_name = "NOT FOUND"
		logging.critical("No product_name found in README for {} from {}".format(self.name, readme_url))
		...
		if (len(self.repos) != 0 and self.repos is not None):
			for repo in self.repos:
				...
				if "ibmcom" in repo:#repos = ibmcom/image_name or image_name or ibmcom
					if "/" in repo:
						self.clean_repos.append(repo.split("/",1)[0])
						self.images.append(repo.split("/",1)[1])
					...
					if "/" in repo:
						...
						self.clean_repos.append("ibmcom")
	...
	def repo_crawl(self, hub_list):
		...
		for i in range(len(self.images)):
			...
			if self.is_bad == True:
				logging.warning('%s contains a weird image from index.yaml', self.name)
				break
			...
			logging.warning('%s: %s  %s ', self.name, name, final_repo)
			...
			# This is the url we will query for all the dockerhub info
	...
	def output_app_keywords(self, f):
		...
		if 'amd64' in self.keywords and 'ppc64le' in self.keywords and 's390x' in self.keywords:
	...
	def generate_output(self):
		...
		logging.info('%s: Num of images: %s Num of tags: %s Num of repos: %s', 
			self.name, str(len(self.images)), str(len(self.tags)), str(len(self.images)))
		...
		with open ('generated_input.yaml', 'a') as outputski:
			if len(self.images) != len(self.tags) or len(self.repos) != len(self.images):
				...
				outputski.write('  ' + self.name + ':\n')
				for i in range(len(self.images)):
					final_repo = 'hub.docker.com/' + self.clean_repos[i] + '/' + str(self.tags[i])
					image = str(self.images[i])

					if str(self.images[i]) == "" or str(self.images[i]) is None:
...
class Image:
	...
	def get_image_tag_names(self):
		...
		for tag in results:
			...
			logging.info('get_image_tag_names: image name: %s tag: %s',
							self.name, self.tags[-1])
	...
	def get_archs(self):
		...
		if self.num_archs == 1:  # I have one arch so ill take it
			...
			for arch_name in range(self.num_archs):
				...
				self.add_arch(wanted_tag["images"][arch_name]["architecture"])
