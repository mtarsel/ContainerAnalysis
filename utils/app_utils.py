...
def parse_both_4(image_results):
    """ parses the repos from a image_results with the format:
{
    'imageType': ['ace'],
    'image':
    ...
    repos = []
    tags = []
    ace_server_dict = image_results["image"][0]
    for name, repo_tag_combo in ace_server_dict.items():
        if repo_tag_combo is not None and ":" in repo_tag_combo:
            repo_tag_list = repo_tag_combo.split(":")
            repos.append(repo_tag_list[0])
            tags.append(repo_tag_list[1])
    return tags, repos
